...
为紧扣“C题”问题一（确定性最优化），本章在明确赛题数据口径与农艺规则的基础上，构建以“年—季—地块—作物”为核心索引的多期混合整数规划模型。我们以经营净收益最大化为目标，并用“面积不超—同季不重叠—作物适宜—产销匹配—轮作与重茬抑制—资源瓶颈—切换成本”七类约束依次收紧可行域。与之配套，结构类图（如图\ref{fig:model1-stack-area}、图\ref{fig:model1-area-by-crop-bar}）揭示作物结构与地块配置；绩效类图（如图\ref{fig:model1-revenue-curve}、图\ref{fig:model1-profit-distribution}）展示收益水平与分布；约束占用类图（如图\ref{fig:model1-resource-util-line}、图\ref{fig:model1-resource-heatmap}、图\ref{fig:model1-sales-vs-capacity}）解释瓶颈限制与产销匹配。本章作为后续鲁棒与滚动章节的基线，用于对比不同风险设定下的收益—风险权衡。

\subsubsection{问题分析（与赛题对齐）}
本节以“题面—附件—模型”的映射方式拆解问题一：
...
上述场景均可在数据口径与参数侧直接调节，并可参见附录算法（\cref{lst:revenue-curve,lst:stack-area}）的实现要点，以便复核口径并保证结论的可复现与可审计。
\paragraph{业务解读与取值逻辑}
...
\paragraph{复现说明}
本章图件可由目录 \texttt{python/figures\_model1/} 下脚本一键复现：\texttt{model1\_stack\_area.py} 生成图~\ref{fig:model1-stack-area}；\texttt{model1\_area\_by\_crop\_bar.py} 生成图~\ref{fig:model1-area-by-crop-bar}；\texttt{model1\_resource\_util\_line.py} 与 \texttt{model1\_resource\_heatmap.py} 分别对应图~\ref{fig:model1-resource-util-line} 与图~\ref{fig:model1-resource-heatmap}；\texttt{model1\_sales\_vs\_capacity.py} 对应图~\ref{fig:model1-sales-vs-capacity}；\texttt{revenue\_curve.py} 对应图~\ref{fig:model1-revenue-curve}；\texttt{model1\_profit\_distribution.py} 与 \texttt{model1\_sensitivity\_spider.py} 分别生成图~\ref{fig:model1-profit-distribution} 与图~\ref{fig:model1-sensitivity-spider}；\texttt{model1\_yield\_bar.py} 与 \texttt{model1\_switch\_heatmap.py} 用于基础单产/切换现象的补充可视化。运行环境依赖 Python 3 与常见科学计算栈（如 \texttt{numpy}、\texttt{pandas}、\texttt{matplotlib}）。脚本按“数据预处理与口径统一/数据清洗细则”的口径读取参数与结果，输出至 \texttt{figures/} 目录以便论文直接引用。
