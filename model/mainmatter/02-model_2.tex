...
\paragraph{本章贡献与创新点}
\noindent 本章的贡献与创新点包括：\textbf{可解释的不确定集口径}——提出盒式与预算不确定集的工程化标定与分组口径，使参数 $\Gamma$ 与业务风险承受度直连，并给出“相关性成组—预算约束”的口径映射；\textbf{可解的鲁棒等价模板}——在问题一可行域基础上，给出 Bertsimas–Sim 体例的线性鲁棒重构与对偶化要点，规模线性增长、分钟级可解；\textbf{可复现的评测与选点流程}——构建名义/最坏/蒙特卡洛三情景评测与前沿选点流程，并配套脚本与图表支持审计与复核。
...
\paragraph{代表性鲁棒重构示例（预算不确定集，Bertsimas–Sim \cite{bertsimas2004price}\,体例）}
...
\paragraph{复杂度与求解建议}
为每条被鲁棒化的约束引入 $\theta,\pi$ 等辅助变量后，规模按“约束数+变量数”线性增长，分钟级可解性保持。工程上宜启用求解器预处理与削弱（presolve、probing），并在资源与切换约束上打开隐式 SOS 识别；以 $\Gamma=0$ 的解作为热启动，逐步提升 $\Gamma$，必要时设置松弛上界与迭代时间窗；同时设置 MIPGap 与 TimeLimit，并记录各 $\Gamma$ 的最优值与可行解，以便绘制前沿并支撑选点。
...
可调鲁棒（ARC）\cite{ben2004adjustable}允许部分决策在观测后调整，求解为分段线性但规模更大；分布鲁棒（DRO）\cite{rahimian2019distributionally}在“分布集合”上最坏化，能显式控制覆盖概率与尾部风险。鉴于题目规模与实施成本，本章取“静态鲁棒+预算集”兼顾可解性与业务可解释性，将 ARC/DRO 作为展望方向（见本章小结）。
...
为保证不确定集口径可复核、可解释，我们采用一段式叙述：选择近 $L\in[3,5]$ 年作为窗口，并按“作物—季（必要时含地块）”分组得到样本集 $\mathcal W_{tsk}$；以分位数给出区间（如 $[Q_{\alpha},\,Q_{1-\alpha}]$，或先对原样本 Winsorize 后取上下界）；偏差幅度 $\sigma$ 可取标准差 $\sigma_{tsk}=\mathrm{sd}(\mathcal W_{tsk})$ 或稳健尺度 $\sigma_{tsk}=1.4826\,\mathrm{MAD}(\mathcal W_{tsk})$。预算集中的分量按“作物—季（或地块）”成组，$\Gamma$ 的含义与分组一致；最后在历史滚动窗口上回测违约率与最坏收益下界，微调 $\alpha,\sigma,\Gamma$ 以满足“保底≥阈值、违约≤阈值”的要求。
...
在实际使用中，可按“求解—阈值过滤—帕累托选择”的三步走执行：先在网格 $\Gamma\in\{0,1,2,\ldots\}$ 上求解并缓存 $x^\Gamma$ 与评测指标（$r^{\min}$、$r^{\mathrm{nom}}$、$\widehat r^{\,\mathrm{avg}}$、ViolRate、AvgGap）；随后依据业务阈值“保底≥$X$、违约≤$Y\%$”过滤候选；若多解并列，优先选择切换更少、峰值资源更低者。}
...
常见停增规则包括：
\begin{itemize}
  \item \textbf{阈值法}：取最小的 $\Gamma$ 使得 $r^{\min}(\Gamma)\ge X$ 且 $\text{ViolRate}(\Gamma)\le Y\%$；
  \item \textbf{性价比法}：当边际收益比 $\Delta r^{\min}(\Gamma)/\max\{\Delta r^{\mathrm{nom}}(\Gamma),\varepsilon\}$ 低于预设阈值时停止增加 $\Gamma$；
  \item \textbf{平滑性优先}：当多个 $\Gamma$ 值并列时，选择切换频次更少、峰值资源占用更低的方案。
\end{itemize}

\paragraph{分层 $\Gamma$ 设定（按季/品种）}
工程上，$\Gamma$ 可分层设定以贴合风险不均衡：
在高波动季（如雨季）可取更大的季节层级 $\Gamma_{t,s}$，对价格/单产波动更大的品种可取更大的品种层级 $\Gamma_{t,k}$；并通过上界约束控制 $\sum_{s,k} \Gamma_{t,sk}\le \Gamma_t^{\max}$ 以避免过度保守。
分层方式需与不确定分组一致（如按“作物—季”建组），以保持对偶模板口径不变。

\paragraph{与机会约束的关系（保守外逼近）}
若目标是满足机会约束 $\mathbb{P}\{g(x,\xi)\le0\}\ge1-\alpha$，直接求解在计算上较为困难。在对称区间分布与预算约束下，Bertsimas–Sim 不确定集 $\mathcal{U}_\Gamma$ 可视为对该机会约束的保守外逼近：
\begin{equation}
  \{g(x,\xi)\le0,\;\forall\xi\in\mathcal{U}_\Gamma\}\;\Rightarrow\;\mathbb{P}\{g(x,\xi)\le0\}\ge1-\alpha,\label{rel:chance}
\end{equation}
...
{\setlength{\emergencystretch}{2em}
\textbf{系数不确定、$\le$ 约束}：$\sum_i (\bar a_i+\hat a_i z_i) x_i \le b$, $\sum z_i\le\Gamma$, $z\in[0,1]$。鲁棒等价如下：}
\begin{align}
  &\sum_i \bar a_i x_i + \Gamma\,\theta + \sum_i \pi_i \;\le\; b,\\
  &\pi_i \ge \hat a_i x_i - \theta,\; \pi_i\ge0,\; \theta\ge0.\nonumber
\end{align}
{\setlength{\emergencystretch}{2em}
\textbf{右端不确定、$\le$ 约束}：$\sum_i \bar a_i x_i \le \bar b - \sum_j \hat b_j z_j$, $\sum z_j\le\Gamma$。等价形式为：}
\begin{align}
  &\sum_i \bar a_i x_i + \Gamma\,\theta + \sum_j \pi_j \;\le\; \bar b,\\
  &\pi_j \ge \hat b_j - \theta,\; \pi_j\ge0,\; \theta\ge0.\nonumber
\end{align}
{\setlength{\emergencystretch}{2em}
\textbf{系数不确定、$\ge$ 约束}：$\sum_i (\bar a_i-\hat a_i z_i) x_i \ge c$, $\sum z_i\le\Gamma$（最坏情形降低左端）。对应鲁棒等价为：}
\begin{align}
...
我们对比“确定性 vs. 鲁棒”的收益曲线与结构变化，并给出多维图示。总体上，鲁棒方案的最坏收益显著抬升且波动收敛；确定性方案在名义情景下均值略高但尾部更弱。随 $\Gamma$ 增大，保底收益上升而名义收益小幅让渡，形成可解释的前沿曲线（见图~\ref{fig:model2-gamma-frontier}）。结构上，鲁棒方案在“高不确定季”的配置更克制，切换次数与跨季跳变减少（见图~\ref{fig:model2-switch-heatmap-robust}）。下列图均由自动化流程生成，口径与问题一一致；抽样固定随机种子以保证复现，整体可复现可审计。

\subsubsection{复现实验指引}
{\setlength{\emergencystretch}{3em}\sloppy
\textbf{环境与依赖}：建议 Python $\ge$ 3.9。常用依赖包含 \texttt{numpy}、\texttt{pandas}、\texttt{matplotlib}（或 \texttt{seaborn}），统计检验可选 \texttt{scipy}（复现实验环境已统一配置，具体操作细节不在正文展开）。
\textbf{随机性与种子}：实现中先固定随机种子（如设置 \texttt{SEED} 或调用 \texttt{numpy.random.seed}），再进行抽样或情景生成，以确保图表可重复。
\textbf{审计与复核}：核对生成文件的时间戳与尺寸，并对比论文中的图编号与标题。若存在差异，优先确认脚本参数（特别是 $\Gamma$ 网格与样本量 $N$）与数据口径是否一致。}

\paragraph{图表—脚本映射（可复现）}
为便于审计复核，Model~2 的图均由下列脚本自动生成。图内标签（如 \verb|\label{fig:model2-revenue-curve}|）均已在正文中定义、保持不变。
\begin{table}[htbp]
  \centering
  \small
  ...
  \begin{tabular}{@{}ll@{}}
    \toprule
    图文件（figures/） & 生成脚本（python/figures\_model2/） \\
    \midrule
    model2\_revenue\_curve.pdf & model2\_revenue\_curve.py \\
    model2\_regret\_curve.pdf & model2\_regret\_curve.py \\
    model2\_gamma\_frontier.pdf & model2\_gamma\_frontier.py \\
    model2\_structure\_stack\_area\_gamma.pdf & model2\_structure\_stack\_area\_gamma.py \\
    model2\_violation\_risk\_bar.pdf & model2\_violation\_risk\_bar.py \\
    model2\_resource\_util\_line\_robust.pdf & model2\_resource\_util\_line\_robust.py \\
    model2\_switch\_heatmap\_robust.pdf & model2\_switch\_heatmap\_robust.py \\
    model2\_sensitivity\_spider\_robust.pdf & model2\_sensitivity\_spider\_robust.py \\
    \bottomrule
  \end{tabular}
  \caption{Model~2 图表与生成脚本的对应关系（标签保持不变）}
  \label{tab:model2-fig-script-map}
\end{table}

\paragraph{统计显著性与稳健性检验}
为避免“肉眼评图”的主观性，我们对关键结论进行统计复核：

\textbf{均值比较}：在相同情景集下比较两方案的年度收益均值，采用双样本 $t$ 检验（或配对 $t$ 检验，视抽样设计而定），并报告效应量 Cohen's $d$。在非正态或重尾分布下，以置换检验作为稳健替代。

\textbf{分布差异检验}：对“后悔值”和“最坏分位收益”（如 5\% 分位）使用百分位自助法（bootstrap）构造 95\% 置信区间，并用 Kolmogorov–Smirnov 检验或能量距离检验复核分布差异。

\textbf{违约率分析}：对违约率（ViolRate）进行比例差异的 $z$ 检验或 Fisher 精确检验，并报告绝对差与相对差的区间估计，避免仅依赖 $p$ 值进行判断。

\paragraph{多重比较校正（避免第一类错误膨胀）}
在同时比较多个指标或多个 $\Gamma$ 时，建议进行多重校正：控制家族错误率（FWER）可用 Bonferroni 或 \emph{Holm-Bonferroni}，当指标较多时控制发现率（FDR）可用 \emph{Benjamini–Hochberg}（BH）。具体而言，Bonferroni 将显著性阈值改为 $\alpha/m$（$m$ 为同时检验数），稳健但偏保守；Holm-Bonferroni（推荐于 $m\le5$）按升序排列 $p_{(k)}$，依次比较 $p_{(k)}\le \alpha/(m-k+1)$ 并在首次不通过时停止；BH-FDR（推荐于 $m>5$）给定 FDR 目标 $q$（如 $q=0.10$），取最大 $k$ 使 $p_{(k)}\le (k/m)q$，从而拒绝 $1\ldots k$ 个原假设。报告时应给出校正后的 $p_{\mathrm{adj}}$ 或注明所用方法与 $\alpha/q$，并在附录列出指标清单与 $m$ 的确定口径。以上检验均在相同的抽样情景上进行，避免“基准不一致”带来的比较偏差。
...
\end{equation}
其中 $\mathcal{U}$ 为不确定集，$\Delta z_{tsik}$ 根据相邻季的作物选择定义切换指示变量。对于盒式不确定集 $\mathcal{U}_{\Box}$ 或预算不确定集 $\mathcal{U}_{\Gamma}$，上述极大极小问题~\eqref{obj:model2-minmax} 中的"内层最小化"可通过对偶理论转化为等价的确定性线性约束，从而得到目标函数~\eqref{obj:model2-robust} 的可求解形式。图中鲁棒方案收益曲线在尾部的抬升效应，正是约束右端"保守折减"项发挥作用的直接体现。
\FloatBarrier
...
为便于执行，选点流程概述如下：先以 $\Gamma=0$（确定性）求解作为\emph{基线}，获得收益与风险参照；随后取 $\Gamma\in\{1,2,3,\ldots\}$ 逐点求解并绘制前沿曲线（见图~\ref{fig:model2-gamma-frontier}），观察“保底抬升—均值让渡”的权衡；最后按业务阈值“保底≥$X$、违约≤$Y\%$”筛选推荐点，若存在并列解，优先选择切换更少者。
  \begin{table}[htbp]
    \centering
    \small
...
为减小数值误差与求解不稳定：
统一价格、成本与收益的量纲（如以千元计）以避免系数跨越 $10^4$ 量级；对面积与产量做适度缩放，使典型变量量级落在 $[10^{-1},10^{2}]$；并在求解器中开启比例缩放（scaling）与数值强调（numeric emphasis）等选项以提升数值稳定性。

\subsubsection{影子价（对偶）与业务含义}
{\setlength{\emergencystretch}{2em}% 避免长行溢出（局部）
影子价的业务解读如下：$\theta_{tsk}$（单产鲁棒枢轴）衡量“同季/品种在最坏情形下所需的统一安全折减强度”，\emph{高 $\theta$} 表示该季/品种在不确定下更紧张，宜适度降配高波动品种、增加库存/缓冲面积或在该季限制激进扩张；$\pi_{tsik}$（单产偏差的分量折减）刻画“地块–季–作物”的边际折减量，\emph{高 $\pi$} 指向具体脆弱点，宜对高 $\pi$ 的地块做结构分散或换作更稳健品种；$\eta_{tsk},\;\zeta_{tsk}^{(j)}$（销量上限鲁棒枢轴/分量）反映销售能力在最坏情形下的收紧幅度，\emph{高 $\eta$ 或 $\zeta$} 表明“卖得出去”更具不确定，宜提前锁定订单/渠道并优化采收与出货节奏以避免“超产卖不出”；$\theta^{(p)}_t,\pi^{(p)}_{tsk}$ 与 $\theta^{(c)}_t,\pi^{(c)}_{tsik}$（价格/成本鲁棒模板）对应收益与成本项的保守化强度，\emph{高 $\pi^{(p)}$} 指向价格敏感的品种/季，\emph{高 $\pi^{(c)}$} 指向成本压力大的地块/工序，宜通过中长期合同降低价差风险，并在高成本敏感环节进行工艺与排班优化；读取与呈现方面，可将上述对偶值随季/品种/地块汇总为“热力矩阵”，与决策量 $x,q$ 并列展示（参见符号表~\ref{tab:model2-symbols} 的口径），并用阈值高亮“需要干预的位置”；此外，对偶值受尺度影响，建议以“归一化影子价”（如按名义右端或典型规模标准化）比较，以避免错判。}

\subsubsection{本章小结与展望}
 本章在问题一的可行域与业务规则之上，构建以盒式与预算不确定集为核心的鲁棒优化框架，给出可工程落地的鲁棒等价与求解流程。通过参数 $\Gamma$ 系统刻画“保底收益—上侧空间”的权衡，并以收益曲线、结构变化、违约风险与敏感性等多维证据予以验证。

 \paragraph{局限}
 主要局限包括：参数区间与偏差口径依赖历史或题设，可能低估极端尾部；采用线性模板与静态 $\Gamma$，尚未显式刻画跨期学习与信息更新；未纳入价格—供给的内生反馈与市场博弈，仍属“给定价格/需求”的视角。

 \paragraph{展望}
 后续可：结合数据驱动或分布鲁棒（DRO）与机会约束/风险度量（如 CVaR）；引入可调鲁棒与多阶段鲁棒，刻画“决策—观测—再决策”的动态链条；探索不确定集的自适应分组与 $\Gamma$ 的自动选点（基于信息准则或业务规则）；并与生产执行系统对接，形成“建议—执行—反馈”的闭环，以真实反馈迭代不确定集与模型参数。