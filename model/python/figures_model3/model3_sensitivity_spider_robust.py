...
def _radar(ax, labels, values, color, label):
    ...
    for ang, val in zip(angles, values):
        # Skip top-vertex annotation completely to avoid any overlap with title
        if abs(ang) < 1e-6:
            continue
        r = min(val + 0.05, 0.98)
...
def main() -> None:
    ...
    out_path = fig_dir / 'model3_sensitivity_spider_robust.pdf'

    labels = ['价格弹性', '容量', '单产\n方差', '切换\n惩罚', '单位\n成本']
    vals = [0.6, 0.65, 0.5, 0.45, 0.4]
    ...
    _radar(ax, labels, vals, '#55A868', '滚动+CVaR')
    # Set tick label fontsize after drawing and pull labels slightly inward
    ax.set_xticklabels(labels, fontsize=10)
    ax.tick_params(axis='x', pad=-8)
    ax.set_title('敏感性（滚动+CVaR）', pad=36, fontsize=12, fontweight='bold')

    # Place legend outside on the right to avoid overlap with title
