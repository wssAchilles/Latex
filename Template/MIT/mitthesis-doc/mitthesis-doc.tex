...
\NewDocumentEnvironment{nomenclature*}{O{2em} O{\nomname} O{section} O{#3}}{%
    ...
    \setlength{\nomenwidth}{#1}%
    \MakeLinkTarget*{nomenclature}\csname #3\endcsname *{#2}\addcontentsline{toc}{#4}{#2}%
    \vspace{-\multicolsep}% we don't need to add to the vertical space from the heading
	\begin{multicols}{2}%
    \raggedright
    \begin{list}{}{%
         \setlength{\itemsep}{0pt}%
...
The \texttt{mitthesis} class will insert the content of \texttt{mydesign.tex} at the appropriate point (prior to loading  \texttt{\hrefurl{https://ctan.org/pkg/babel}{babel}}, fonts, or \texttt{\hrefurl{https://ctan.org/pkg/hyperref}{hyperref}}). You should not need to edit the class file.
...
\EntryHeading{Greek letters}
\entry{$\Gamma$}{circulation [$\cramped{\textrm{m}^2}$ $\cramped{\textrm{s}^{-1}}$]}
\entry{$\rho$}{mass density [kg m$\cramped{\textrm{m}^{-3}}$]}
\entry{$\symbfup{\omega}$}{vorticity [$\cramped{\textrm{s}^{-1}}$]}
\end{nomenclature*}
